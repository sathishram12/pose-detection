                                    ...
                                    float *result)
{
  ...
  for (int i = 0; i < n_result_channels; ++i)
  {
    ...
    /*std::cout<<"top_leftl="<<tensor[top_left + c]<<" top_right="<<tensor[top_right + c];
    std::cout<<" bottom_left="<<tensor[bottom_left + c]<<" bottom_right="<<tensor[bottom_right + c] <<std::endl;*/
                            ...
                            const int mid_short_offset_refinement_steps)
{
  ...
  //std::cout<<x<<","<<y<<" " <<target_id<<std::endl;
                         ...
                         PoseKeypointScores *keypoint_scores)
{
  ...
  while (!decode_queue.empty())
  {
    ...
    for (int j = 0; j < num_children; ++j)
    {
      ...
      /*std::cout<<"+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"<<std::endl;
      std::cout<<current_keypoint.point.y<<" "<<current_keypoint.point.x<<" "<<current_keypoint.id<<" " <<current_keypoint.score<<std::endl;
      std::cout<<child_point.y<<" "<<child_point.x<<" "<<child_id<<" " <<child_score<<std::endl;
      std::cout<<"+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"<<std::endl;*/
        ...
        if (score >= score_threshold)
        {
          ...
          if (local_maximum)
          {
            ...
            /*if (Sigmoid(score) > 0.5 )
            {
              ...
              std::cout<<" x="<<x_refined*16<<" y="<<y_refined*16<<" pos="<<j<<" "<<Sigmoid(score) <<std::endl;
            }*/
...
namespace posenet_decoder_op
{
    ...
    for (size_t index : decreasing_indices)
    {
      ...
      for (int k = 0; k < kNumKeypoints; ++k)
      {
        pose_keypoints[pose_counter].keypoint[k].y =
            scratch_poses[index].keypoint[k].y ;
        pose_keypoints[pose_counter].keypoint[k].x =
            scratch_poses[index].keypoint[k].x ;
      }
  ...
  std::string FormatPosenetOutput(const std::vector<Pose> &poses)
  {
    ...
    for (size_t i = 0; i < poses.size(); ++i)
    {
      ...
      for (int j = 0; j < posenet_decoder_op::kNumKeypoints; ++j)
      {
...
int main()
{
  ...
  const int score_threshold = 0.20000000298023224;
  const int mid_short_offset_refinement_steps = 5;
  const int stride = 16;
  const float nms_radius = 10.0f / stride;
  float *pose_keypoints_data = new float[max_detections * posenet_decoder_op::kNumKeypoints * 2];
  float *pose_keypoint_scores_data = new float[max_detections * posenet_decoder_op::kNumKeypoints];
  float *pose_scores_data = new float[max_detections * posenet_decoder_op::kNumKeypoints];
  int pose_count_data = 0;
  float threshold = 0.7;
  pose_count_data = posenet_decoder_op::DecodeAllPoses(heatmap, short_offsets, mid_offsets, height, width,
                                                       max_detections, score_threshold,
                                                       mid_short_offset_refinement_steps,
                                                       nms_radius, stride,
                                                       reinterpret_cast<posenet_decoder_op::PoseKeypoints *>(pose_keypoints_data),
                                                       reinterpret_cast<posenet_decoder_op::PoseKeypointScores *>(pose_keypoint_scores_data),
                                                       pose_scores_data);

  int pose_count = static_cast<int>(pose_count_data);
  std::vector<Pose> poses;
  poses.reserve(pose_count);
  for (int i = 0; i < pose_count; ++i)
  {
    if (pose_scores_data[i] < threshold)
      continue; // Skip poses that are less than the threshold.
    Pose pose{};
    pose.score = pose_scores_data[i];
    float *pose_keypoints = pose_keypoints_data + (i * posenet_decoder_op::kNumKeypoints * 2);
    float *keypoint_scores = pose_keypoint_scores_data + (i * posenet_decoder_op::kNumKeypoints);
    for (int j = 0; j < posenet_decoder_op::kNumKeypoints; ++j)
    {
  ...
  std::cout<<"----------------------------------------------------"<<std::endl;
  std::cout<<posenet_decoder_op::FormatPosenetOutput(poses).c_str();
  std::cout<<"----------------------------------------------------"<<std::endl;
return 0;
}